...
$(function () {
	...
	$('button.newgame').click(function () {
		...
		$('.caixa.login').find('form').find('input[name="name"]').val(Cookies.get('name')).end().find('input[name="pass"]').val(Cookies.get('pass')).end().submit();
	});
	...
	$('.frame.arenas').find('.btn.next').click(function () {
		...
		$('.frame.chars').fadeIn(250, function () {
			$(this).find('ul.chars').append(['red', 'yellow'].map(function (char) {
				return $('<li>').append($('<input>', {
					'type': 'radio',
					'name': 'char',
					'id': 'char-' + char,
					'value': char
				}).prop('checked', true), $('<label>', {
					'for': 'char-' + char
				}).append($('<img>', {
